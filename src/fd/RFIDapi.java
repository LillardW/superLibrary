...
public class RFIDapi {
	...
	public String testreadandwrite() {
		...
		for(int i =0;i<pwdb.length;i++)
			System.out.println("写入标签密码："+pwdb[i]);
		 ...
		 * blkcnt,byte[] data, byte[] accesspasswd, short timeout); ant 操作的单天线
		 * bank 表示区域 0表示保留区 1表示epc区 2表示tid区 3表示user区 address 表示地址块， 注意epc区从第二块开始
		 * blkcnt 表示读块数 data 存放数据的字节，应该不小于blkcnt*2 accesspwd 表示密码，默认"00000000"
		 * 8个十六进制字符 timeout 操作超时时间
		 * 
		...
		for (int i = 0; i < 12; i++) {
			...
			System.out.println("原始数据位："+datar[i]);
		}
		System.out.println(er.toString() + " 读取到的数据：" + str1.toUpperCase());
		System.out.println(er.toString() + " 读取到的原数据：" + str2.toUpperCase());
...
return str1.toUpperCase();
	...
	public byte[] testreadandwrite3() {
		...
		String pwd = "12345678";
		 ...
		 0x96,(byte) 0xaa,(byte) 0xbb,(byte) 0xcc};
		byte[] pwdb = new byte[4];
		//System.out.println("3333需要写入的数据是：");
		for (int i =0;i<data.length;i++){
			//System.out.print(data[i]+" ");
		}
...
return datar;
	...
	public void testinitreader() {
		// 创建读写器
		/*
		 * 根据天线口连接读写器 src 是地址 ip地址或者串口号 rtype 是天线口数，4口天线传入4 返回类型：READER_ERR
		 * ,MT_OK_ERR表示正常，其他表示错误
		 */
		...
		// yj 屏蔽 System.out.println(er.toString());

		/*
		 * 构建天线组功率：AntPowerConf 成员： AntPower数组 antcnt表示天线个数 AntPower类型 antid 天线号
		 * readPower 读功率 writePower 写功率
		 */
		...
		/*
		 * 设置是否检查天线 当参数值传入1的时候表示要检查，0表示不检查
		 */
	...
	public String testinventory1() {
		...
		/*
		 * 
		 * 设置频率，对于某天线设置频率表可以提搞读效果 HoptableData_ST 天线频率表类 lenhtb 表示频率表长度 htb
		 * 数组int类型，表示具体频点 HoptableData_ST hdst=Jreader.new HoptableData_ST();
		...
		/*
		 * ants 轮询使用的天线，即连接天线口的天线 antcnt 使用的天线个数 timeout 读标签的时间 pTInfo 存放标签数据的数组
	...
	public void testlocktag() {
		...
		er = Jreader.WriteTagData(1, (char) 1, 2, new byte[] { 0x11, 0x22 }, 2,
				null, (short) 1000);
		// yj屏蔽 ：System.out.println("no pwd write"+er.toString());

		// 写数据
		er = Jreader.WriteTagData(1, (char) 1, 2, new byte[] { 0x11, 0x22 }, 2,
				data, (short) 1000);
		// yj屏蔽 ：System.out.println("pwd write"+er.toString());
	...
	public void testrparams() {
		...
		/*
		 * //yj屏蔽 ： for(int i=0;i<hdst2.lenhtb;i++) {
		...
		/*
		 * //yj屏蔽 ： System.out.print("ip:"+rip2.ip.length+" ");
		...
		er = Jreader.ParamGet(Mtr_Param.MTR_PARAM_RF_ANTPOWER, apcf2);
		/*
		 * //yj屏蔽 ： for(int i=0;i<apcf2.antcnt;i++) {
		 * System.out.print("antid:"+apcf2.Powers[i].antid);
		 * System.out.print(" rp:"+apcf2.Powers[i].readPower);
		 * System.out.print(" wp:"+apcf2.Powers[i].writePower);
		 * System.out.println(); }
		...
		// yj屏蔽 ：System.out.println("max:"+valso[0]);
		...
		/*
		 * //yj屏蔽 ： for(int i=0;i<ipst2.potlcnt;i++)
	...
	/*
	 *YJ整合 2015-4-19 start
	 */
	public String write(String input) {
		testinitreader();
		String pwd = "12345678";
		// 10进制换16进制，数组用16进制
		// 注意修改的数组长度datalen，WriteTagData()方法的第5个参数，12为全部数组长度
